...
void updateState() {
  ...
  if (g_isLidClosed) {
    if (g_secondaryMonitorsCount > 0) {
      ...
      HyprlandAPI::invokeHyprctlCommand("keyword", std::format("monitor {}, disable", g_mainMonitorName));
    } else if (!g_lockCommand.empty()) {
      logInfo("Locking session");
      HyprlandAPI::invokeHyprctlCommand("keyword", std::format("monitor {}", g_mainMonitorConfig));
      HyprlandAPI::invokeHyprctlCommand("dispatch", g_lockCommand);
      HyprlandAPI::invokeHyprctlCommand("dispatch", std::format("dpms off {}", g_mainMonitorName));
    }
  } else {
    logInfo("Enabling main monitor");
    HyprlandAPI::invokeHyprctlCommand("keyword", std::format("monitor {}", g_mainMonitorConfig));
    HyprlandAPI::invokeHyprctlCommand("dispatch", std::format("dpms on {}", g_mainMonitorName));
  } 
}
...
void monitorAdded(void*, SCallbackInfo&, std::any any) {
  ...
  if (monitor->szName != g_mainMonitorName && monitor->szName != kFallbackMonitorName) {
    if (++g_secondaryMonitorsCount > 0) {
      updateState();
    }
  }
...
void monitorRemoved(void*, SCallbackInfo&, std::any any) {
  ...
  logInfo(std::format("Monitor[{}] removed: [{}]", monitor->ID, monitor->szName));
  if (monitor->szName != g_mainMonitorName && monitor->szName != kFallbackMonitorName) {
...
void configReloaded(void*, SCallbackInfo&, std::any any) {
  auto* const mainMonitorConfig = (Hyprlang::STRING const*)HyprlandAPI::getConfigValue(
      PHANDLE, 
      "plugin:hyprlid:laptop_monitor")->getDataStaticPtr();
  ...
  auto* const lockCommadConfig = (Hyprlang::STRING const*)HyprlandAPI::getConfigValue(
      PHANDLE, 
      "plugin:hyprlid:lock_command")->getDataStaticPtr();
  ...
  auto* const DEBUG_FLAG = (Hyprlang::STRING const*)HyprlandAPI::getConfigValue(
      PHANDLE, 
      "plugin:hyprlid:debug")->getDataStaticPtr();
  ...
  updateState();
  logInfo(std::format("Main monitor[{}] = {}", g_mainMonitorName, g_mainMonitorConfig));
}
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
  ...
  if (HASH != GIT_COMMIT_HASH) {
    logErr("Failed to initialize: Version mismatch (headers ver is not equal to running hyprland ver)");
  ...
  if (!HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprlid:laptop_monitor", Hyprlang::STRING{""})) {
    logErr("Failed to add a config value");
    throw std::runtime_error(std::format("{} {}", kTag, "Failed to add config value"));
  }
  
  if (!HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprlid:debug", Hyprlang::STRING{"false"})) {
    logErr("Failed to add a config value");
    throw std::runtime_error(std::format("{} {}", kTag, "Failed to add config value"));
  }

  if (!HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprlid:lock_command", Hyprlang::STRING{""})) {
    logErr("Failed to add a config value");
    throw std::runtime_error(std::format("{} {}", kTag, "Failed to add config value"));
  }

  g_configReloadedCallback = HyprlandAPI::registerCallbackDynamic(PHANDLE, "configReloaded", configReloaded);
  if (g_configReloadedCallback == nullptr) {
    logErr("Failed to subscribe to configReloaded event");
    throw std::runtime_error(std::format("{} {}", kTag, "Failed to subscribe to configReload event"));
  }

  g_monitorAddedCallback = HyprlandAPI::registerCallbackDynamic(PHANDLE, "monitorAdded", monitorAdded);
  ...
  g_monitorRemovedCallback = HyprlandAPI::registerCallbackDynamic(PHANDLE, "monitorRemoved", monitorRemoved);
  ...
  return {"hyprlid", "A plugin to manage laptop lid open/close actions in a multi-monitor setup", "vkovtash", "1.0"};
}
